...
In this optimization method, we define volume levels that determine the number of cells into which the volume should be divided. At level 0, we divide the volume into ![N x N x N](https://latex.codecogs.com/svg.latex?N%20%5Ctimes%20N%20%5Ctimes%20N) cells, where ![N = 2^M](https://latex.codecogs.com/svg.latex?N%20=%202%5EM). Here, ![M + 1](https://latex.codecogs.com/svg.latex?M%20+%201) represents the number of levels, and the levels are indexed from 0 to ![M](https://latex.codecogs.com/svg.latex?M). On level 1, we increase the size of cells so that each cell is equal in size to 8 cells from the previous level. On this level, the volume is divided into ![N/2 x N/2 x N/2](https://latex.codecogs.com/svg.latex?%5Cfrac%7BN%7D%7B2%7D%20%5Ctimes%20%5Cfrac%7BN%7D%7B2%7D%20%5Ctimes%20%5Cfrac%7BN%7D%7B2%7D) cells. We repeat this process for each level until the volume contains only one cell.
